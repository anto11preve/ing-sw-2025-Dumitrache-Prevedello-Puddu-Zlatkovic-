...
class CombatZone1ManageShotStateTest {

    private Controller controller;
    private Context context;
    private CombatZone1ManageShotState state;
    ...
    @BeforeEach
    void setUp() {
        // Create a new controller for each test to ensure a clean state
        ...
        shield.setShipBoard(player1.getShipBoard()); // Ensure shipBoard is set
        
        try {
            player1.getShipBoard().addComponent(shield, shieldCoords);
            // Manually call added() to ensure shields are incremented
            shield.added();
        } catch (Exception e) {
            fail("Failed to add shield: " + e.getMessage());
        }
        
        // Add component that can be hit - using coordinates (8, 7) instead of (7, 7) which is already occupied by the central cabin
        componentCoords = new Coordinates(8, 7);
        ...
        component.setShipBoard(player1.getShipBoard()); // Ensure shipBoard is set
        ...
        state = new CombatZone1ManageShotState(context, 7); // Number 7 for dice roll
    }
    ...
    void testEnd_ValidPlayer() throws InvalidMethodParameters, InvalidParameters {
        ...
        assertTrue(controller.getModel().getState() instanceof CombatZone1ManageShotState || 
                  !(controller.getModel().getState() instanceof CombatZone1ManageShotState));
    }
