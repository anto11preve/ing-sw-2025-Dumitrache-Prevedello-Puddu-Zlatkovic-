...
public class SlaversStateBatteryRemoved extends State {
    ...
    public SlaversStateBatteryRemoved(Controller controller, Slavers card, List<Player> penalizedPlayers, int declaredPower, int batteriesRequired) {
            ...
            case "removeBattery":
                if(Objects.equals(playerName, players.getFirst().getName())){
                    ...
                    if(batteriesRequired > 0){  //se devo selezionare un altro compartment
                        controller.setState(new SlaversStateBatteryRemoved(controller, card, penalizedPlayers, declaredPower, batteriesRequired));
                    } else {
                        if (declaredPower > card.getFirePower()) {  //se vinci
                            ...
                            if(players.isEmpty()){
                                ...
                                controller.setState(new SlaversStatePowerDeclaration(controller, card, penalizedPlayers));
                            }
                        }
                    }
                }
