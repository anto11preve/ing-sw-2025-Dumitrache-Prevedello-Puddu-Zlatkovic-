...
public class ShipBoard {
            ...
            for (int y = 0; y < COLS; y++) {
                ...
                for (int[] d : dirs) {
                    ...
                    if (nx >= 0 && nx < ROWS && ny >= 0 && ny < COLS && components[nx][ny] != null) {
                        ...
                        ConnectorType otherConn = components[nx][ny].getConnectorAt(getOppositeSide(side));
    ...
    /**
     * Calculates the total thrust produced by all engines that are correctly oriented
     * to the specified rear direction of the ship. Only engines facing the designated rear
     * direction are considered active, and their individual power contributions are summed.
     *
     * This is critical for determining the ship's movement speed and initiative in certain events.
     *
     * @param shipRear the direction that represents the rear of the ship (used for orientation check)
     * @return the total thrust value provided by properly oriented engines
     */
    public int calculateThrust(Direction shipRear) {
        int thrust = 0;
        for (int row = 0; row < ROWS; row++) {
    ...
    /**
     * Determines if the component at the specified position is protected by an active shield generator
     * from the direction corresponding to the incoming side of the attack. The method converts the logical
     * incoming side (e.g. FRONT, LEFT) into a game direction and scans neighboring cells to find any
     * active shield generators facing that direction.
     *
     * This is used to prevent damage to components from external threats like meteors or laser fire.
     *
     * @param pos the position of the component being evaluated for shield protection
     * @param incomingSide the logical side from which the attack originates
     * @return true if there is a shield generator in the adjacent tile facing the incoming direction; false otherwise
     */
    public boolean isProtectedByShield(Position pos, Side incomingSide) {
        int row = pos.getX();
        int col = pos.getY();
    ...
    /**
     * Applies damage to the component located at the specified position on the grid.
     * Before removing the component, the method checks if it is protected by an adjacent shield generator
     * facing the direction of the incoming threat. If not protected, the component is destroyed (set to null).
     *
     * This simulates the effect of external hits (like meteors or attacks) during the adventure phase.
     *
     * @param pos the grid position where the damage is applied
     * @param incomingSide the side of the component from which the damage is approaching
     */
    public void applyDamage(Position pos, Side incomingSide) {
        int row = pos.getX();
        int col = pos.getY();
